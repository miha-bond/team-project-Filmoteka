...
function viewQue() {
  ...
  currentP = currentP.filter(el => queStorageData.includes(el.id));
  refs.galleryItem.innerHTML = '';
...
function viewWt() {
  ...
  currentP = currentP.filter(el => watchedStorageData.includes(el.id));
  refs.galleryItem.innerHTML = '';
  currentP.map(createMarkupElement);
  refs.galleryItem.addEventListener('click', showCard);
}
...
function initLibrary() {
  ...
  if (
    ...
    (load('watched').length !== 0 && !load('queue'))
  ) {
    ...
    (load('queue').length !== 0 && !load('watched'))
  ) {
...
function libraryClassInspect() {
  if (document.querySelector('[data-watched]').classList.contains('is-on')) {
    ...
    document.querySelector('[data-queue]').classList.contains('is-on')
  ) {
